...
public interface HaberRepository extends JpaRepository<Haber, Long> {
    ...
    @Query("SELECT h FROM Haber h WHERE h.kategori.id = :kategoriId AND " +
            "(LOWER(h.baslik) LIKE LOWER(CONCAT('%', :kelime, '%')) " +
            "OR LOWER(h.icerik) LIKE LOWER(CONCAT('%', :kelime, '%')))")
    List<Haber> findByKategoriVeKelime(Long kategoriId, String kelime);
